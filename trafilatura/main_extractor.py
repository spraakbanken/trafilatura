...
from .settings import TAG_CATALOG, ExtractOptions
from .utils import (
...
def handle_titles(element: _Element, options: ExtractOptions) -> Optional[_Element]:
...
def handle_formatting(element: _Element, options: ExtractOptions) -> Optional[_Element]:
...
def process_nested_elements(
    child: _Element, new_child_elem: _Element, options: ExtractOptions
) -> None:
...
def define_newelem(
    processed_elem: _Element, orig_elem: _Element, options: ExtractOptions
) -> None:
    ...
    if processed_elem is not None:
        ...
        for processed_child in processed_elem:
            ...
            childchildelem.text, childchildelem.tail = (
...
def handle_lists(element: _Element, options: ExtractOptions) -> Optional[_Element]:
...
def handle_quotes(element: _Element, options: ExtractOptions) -> Optional[_Element]:
    ...
    processed_element = Element(element.tag)
    for child in element.iter("*"):
        processed_child = process_node(
            child, options
        )  # handle_textnode(child, comments_fix=True)
        if processed_child is not None:
...
def handle_other_elements(
    element: _Element, potential_tags: Set[str], options: ExtractOptions
) -> Optional[_Element]:
    ...
    if element.tag == "div":
        ...
        processed_element = handle_textnode(
            element, options, comments_fix=False, preserve_spaces=True
        )
        if (
            processed_element is not None
            and text_chars_test(processed_element.text) is True
        ):
...
def handle_paragraphs(
    ...
    for child in element.iter("*"):
        ...
        processed_child = handle_textnode(
            child, options, comments_fix=False, preserve_spaces=True
        )
        if processed_child is not None:
...
def handle_table(
    table_elem: _Element, potential_tags: Set[str], options: ExtractOptions
) -> Optional[_Element]:
        ...
        if subelement.tag == "tr":
            ...
            if len(subelement) == 0:
                ...
                if processed_cell is not None:
                    new_child_elem.text, new_child_elem.tail = (
                        processed_cell.text,
                        processed_cell.tail,
                    )
            else:
                ...
                for child in subelement.iterdescendants():
                    if child.tag in TABLE_ALL:
                        ...
                        processed_subchild = handle_textnode(
                            child, options, preserve_spaces=True, comments_fix=True
                        )
                    # todo: lists in table cells
                        ...
                        processed_subchild = handle_textelem(
                            child, potential_tags.union(["div"]), options
                        )
                    # add child element to processed_element
                    if processed_subchild is not None:
                        define_newelem(
                            processed_subchild, new_child_elem, options=options
                        )
                    child.tag = "done"
            # add to tree
            if new_child_elem.text or len(new_child_elem) > 0:
...
def handle_image(
    element: Optional[_Element], options: Optional[ExtractOptions] = None
) -> Optional[_Element]:
...
def recover_wild_text(
    ...
    potential_tags: Any = TAG_CATALOG,
) -> _Element:
...
def prune_unwanted_sections(
    tree: HtmlElement, potential_tags: Set[str], options: ExtractOptions
) -> HtmlElement:
...
def _extract(
    tree: HtmlElement, options: ExtractOptions
) -> Tuple[_Element, str, Set[str]]:
    # init
...
def extract_content(
    cleaned_tree: HtmlElement, options: ExtractOptions
...
def process_comments_node(
    elem: _Element, potential_tags: Set[str], options: ExtractOptions
) -> Optional[_Element]:
