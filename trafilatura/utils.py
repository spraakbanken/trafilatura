...
def check_html_lang(
    tree: HtmlElement, target_language: str, strict: bool = False
) -> bool:
    ...
    for attr in TARGET_LANG_ATTRS:
        ...
        if elems:
            if any(
                target_language in RE_HTML_LANG.split(elem.get("content", "").lower())
                for elem in elems
            ):
                return True
    ...
    if strict:
        ...
        if elems:
            if any(
                target_language in RE_HTML_LANG.split(elem.get("lang", "").lower())
                for elem in elems
            ):
                return True
...
def language_filter(
    temp_text: str, temp_comments: str, target_language: str, docmeta: Any
) -> Tuple[bool, Any]:
