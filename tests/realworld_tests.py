...
def load_mock_page(
    url, xml_flag=False, langcheck=None, tei_output=False, formatting=False, links=False
):
    """load mock page from samples"""
    try:
        with open(
            os.path.join(TEST_DIR, "cache", MOCK_PAGES[url]), "r", encoding="utf-8"
        ) as inputf:
...
@pytest.mark.parametrize(
    "xmloutput,formatting", [(True, False), (False, False), (False, True)]
)
def test_extract(xmloutput, formatting):
    """test extraction from HTML"""
    do_load_page = functools.partial(
        load_mock_page, xml_flag=xmloutput, formatting=formatting
    )
    ...
    assert (
        "Impressum" not in result
        and "Anbieter von Fernwärme haben innerhalb ihres Leitungsnetzes ein Monopol"
        in result
    )
    ...
    if xmloutput is False:
        assert (
            "Wir bearbeiten alle Leistungsbilder" in result
            and "Brückenbau" not in result
        )
    ...
    assert (
        "Kapitel 121 ist" in result
        and "Besucher online" not in result
        and "Kommentare schreiben" not in result
    )
    ...
    assert (
        "Zweitens wird der Genderstern" in result
        and "alldem leider – nichts." in result
    ...
    assert (
        "Es war gesagt," in result
        and "Symbol auf dem Finger haben" in result
        and "Aufrufe:" not in result
    )
    ...
    assert (
        ...
        and "Bahn-Konkurrenten wie Flixbus fürchten durch den geplanten Deutschlandtakt"
        in result
    ...
    assert (
        "Matthias Weber ist ERP-Experte mit langjähriger Berufserfahrung." not in result
    )
    assert "Die Top 5 digitalen Trends für den Mittelstand" not in result
    assert (
        ", leading edge," not in result
    )  # and 'Lesen Sie hier einen weiteren spannenden Beitrag' not in result
...
@pytest.mark.parametrize(
    ("url", "expected"),
    [("https://data.riksdagen.se/dokument/H2B51.html", ["AP-fonden", "2014-01-29"])],
)
def test_extract_single_url(
    url: str, expected: list[str], xmloutput: bool, formatting: bool
) -> None:
...
def test_extract_links_formatting():
    result = load_mock_page(
        "http://www.pcgamer.com/2012/08/09/skyrim-part-1/", formatting=True, links=True
    )
...
def test_pages():
    ...
    metadata = extract_metadata(
        load_mock_page_meta("https://creativecommons.org/about/")
    )
    ...
    metadata = extract_metadata(
        load_mock_page_meta("https://www.creativecommons.at/faircoin-hackathon")
    )
    ...
    metadata = extract_metadata(
        load_mock_page_meta("https://www.befifty.de/home/2017/7/12/unter-uns-montauk")
    )
    ...
    metadata = extract_metadata(
        load_mock_page_meta("https://www.soundofscience.fr/1927")
    )
    ...
    assert (
        metadata.url == "http://www.laviedesidees.fr/L-evaluation-et-les-listes-de.html"
    )
    ...
    assert (
        metadata.title == "Thousands of UK academics 'treated as second-class citizens'"
    )
    ...
    assert (
        metadata.title == "What is Rust and why is it so popular? - Stack Overflow Blog"
    )
    ...
    metadata = extract_metadata(load_mock_page_meta(url))
    assert (
        "Berlin confronts Germany's colonial past with new initiative" in metadata.title
    )
