...
def test_fetch():
    ...
    if HAS_PYCURL:
        ...
        assert _handle_response(url, response1, True, DEFAULT_OPTS) == _handle_response(
            url, response, True, DEFAULT_OPTS
        )
    ...
    new_config = (
        use_config()
    )  # get a new config instance to avoid mutating the default one
    ...
    if HAS_PYCURL:
        assert (
            _send_pycurl_request(
                "https://httpbun.com/redirect/1", True, False, new_config
            )
            is None
        )
...
def test_queue():
    ...
    with patch.object(sys, "argv", testargs):
        args = parse_args(testargs)
    inputurls = [
        f"https://httpbun.com/status/{i}" for i in (301, 304, 200, 300, 400, 505)
    ]
    url_store = add_to_compressed_dict(inputurls)
    args.archived = True
