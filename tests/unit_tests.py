...
def load_mock_page(url, xml_flag=False, langcheck=None, tei_output=False):
    ...
    try:
        with open(
            path.join(TEST_DIR, "resources", MOCK_PAGES[url]), "r", encoding="utf-8"
        ) as inputf:
...
def test_input():
    ...
    # output format
    assert (
        extract("<html><body><p>ABC</p></body></html>", output_format="xml") is not None
    )
    with pytest.raises(AttributeError):
        assert (
            extract("<html><body><p>ABC</p></body></html>", output_format="xyz")
            is not None
        )
    assert (
        bare_extraction("<html><body><p>ABC</p></body></html>", output_format="python")
        is not None
    )
    with pytest.raises(AttributeError):
        assert (
            bare_extraction("<html><body><p>ABC</p></body></html>", output_format="xyz")
            is not None
        )
...
def test_xmltocsv():
    ...
    assert extract(
        mystring, output_format="csv", include_comments=False, config=ZERO_CONFIG
    ).endswith("\tnull\r\n")
...
def test_tojson():
    ...
    result = extract(
        mystring, output_format="json", config=ZERO_CONFIG, with_metadata=True
    )
    assert (
        result.endswith("}") and '"fingerprint":' in result and '"language":' in result
    )
    assert extract(
        mystring, output_format="json", include_comments=False, config=ZERO_CONFIG
    ).endswith("}")
...
def test_exotic_tags(xmloutput=False):
    ...
    # naked div with <lb>
    assert "1.\n2.\n3." in extract(
        "<html><body><main><div>1.<br/>2.<br/>3.<br/></div></main></body></html>",
        fast=True,
        config=ZERO_CONFIG,
    )
    # HTML5: <details>
...
<html>
  ...
  <body>
    ...
    assert (
        extract(
            htmlstring, include_formatting=True, include_links=True, include_images=True
        )
        is not None
    )
...
<html>
  ...
  <body>
    ...
    params = [
        common,
        {**common, "favor_precision": True},
        {**common, "favor_recall": True},
    ]
    for p in params:
        result = extract(htmlstring, **p)
        assert "em improperly wrapping p here" in result and result.endswith(
            "Text here"
        )
...
def test_formatting():
    ...
    my_document = html.fromstring(
        "<html><body><p>This here is the text.<br/></p></body></html>"
    )
    ...
    my_result = extract(
        my_document, output_format="xml", include_formatting=True, config=ZERO_CONFIG
    )
    ...
    my_result = extract(
        my_document, output_format="txt", include_formatting=True, config=ZERO_CONFIG
    )
    ...
    assert '<hi rend="#b">' in etree.tostring(
        bare_extraction(my_string, output_format="markdown", config=ZERO_CONFIG).body,
        encoding="unicode",
    )

    meta_string = (
        "<html><head><title>Test</title></head><body><p>ABC.</p></body></html>"
    )
    meta_result = extract(
        meta_string, output_format="markdown", config=ZERO_CONFIG, with_metadata=True
    )
...
```"""
    ...
    assert (
        "<p>" in my_result and '<hi rend="#b">Wild text</hi>' in my_result
    )  # no rend so far
    my_document = html.fromstring(
        "<html><body><article><div><strong>Wild text</strong></div></article></body></html>"
    )
    ...
    my_result = extract(
        my_document, output_format="xml", include_links=True, config=ZERO_CONFIG
    )
    ...
    """)
    my_result = extract(
        my_document, output_format="markdown", include_links=True, config=ZERO_CONFIG
    )
    assert (
        my_result
    ...
    my_result = extract(
        copy(my_document), fast=True, include_formatting=False, config=ZERO_CONFIG
    )
    assert (
        my_result
    ...
    my_result = extract(
        copy(my_document), fast=True, include_formatting=True, config=ZERO_CONFIG
    )
    assert (
        my_result
        == "**bold**, *italics*, `tt`, ~~deleted~~, __underlined__, link and additional text to bypass detection."
    )
    ...
    my_document = html.fromstring(
        "<html><body><p>AAA, <p>BBB</p>, CCC.</p></body></html>"
    )
    my_result = extract(
        my_document,
        output_format="xml",
...
def test_extract_with_metadata():
    ...
    """)
    parsed_doc = extract_with_metadata(
        my_document, output_format="txt", include_formatting=True, fast=True, url=url
    )
    content = parsed_doc.text
    assert "AAA" in content and "BBB" in content and "CCC" in content
    assert (
        url == parsed_doc.url and parsed_doc.date is None and parsed_doc.title is None
    )
    ...
    assert (
        url == parsed_doc.url
        and "2021-05-24" == parsed_doc.date
        and "title" == parsed_doc.title
    )

    parsed_doc = extract_with_metadata(my_document, output_format="xml")
    assert (
        "AAA, BBB , CCC." == parsed_doc.raw_text
        and "ee7d2fb6fcf2837d" == parsed_doc.fingerprint
    )
    content = parsed_doc.text
    assert "AAA" in content and "BBB" in content and "CCC" in content
...
def test_external():
    ...
    if LANGID_FLAG is True:
        doc = html.fromstring(
            "<html><body>" + "<p>Non è inglese.</p>" * 20 + "</body></html>"
        )
    ...
    assert res is not None and "Features" in res
...
def test_images():
    ...
    assert "![Example image](test.jpg)" in extract(
        teststring, include_images=True, fast=True
    )
    ...
    assert (
        myimage is not None
        and "alt" in myimage.attrib
        and "src" in myimage.attrib
        and myimage.get("src").startswith("http")
    )
...
def test_links():
    ...
    assert (
        handle_formatting(
            html.fromstring('<a href="testlink.html">Test link text.</a>'), options
        )
        is not None
    )
    # empty link
    ...
    assert (
        "[Test link text.](testlink.html) This part of the text has to be long enough."
        in extract(copy(mydoc), include_links=True, fast=True, config=ZERO_CONFIG)
    )
    # relative link conversion
    assert (
        "[Test link text.](https://www.example.com/testlink.html) This part of the text has to be long enough."
        in extract(
            copy(mydoc),
    ...
    assert "[link](testlink.html)" in extract(
        teststring, include_links=True, fast=True, config=ZERO_CONFIG
    )
    ...
    mydoc = html.fromstring(
        f"<html><body><article><p><a>f{'abcd' * 20}</a></p></article></body></html>"
    )
    assert "abc" in extract(
        copy(mydoc), fast=True, config=ZERO_CONFIG, favor_precision=False
    )
...
def test_tei():
    ...
    result1 = extract(
        teststring, "mocked", fast=True, output_format="xmltei", tei_validation=False
    )
    result2 = extract(
        teststring, "mocked", fast=True, output_format="xmltei", tei_validation=True
    )
    ...
    xml_doc = etree.fromstring(
        "<TEI><text><body><div><div>text1<p>text2</p></div></div></body></text></TEI>"
    )
    cleaned = xml.check_tei(xml_doc, "fake_url")
    result = [(elem.tag, elem.text) for elem in cleaned.find(".//div").iter()]
    ...
    assert result == expected
    xml_doc = etree.fromstring(
        "<TEI><text><body><div><div>text1<head>text2</head></div></div></body></text></TEI>"
    )
    cleaned = xml.check_tei(xml_doc, "fake_url")
    result = [(elem.tag, elem.text) for elem in cleaned.find(".//div").iter()]
    ...
    assert result == expected
    xml_doc = etree.fromstring(
        "<TEI><text><body><div><div>text1<p>text2</p></div>has to be there</div></body></text></TEI>"
    )
    cleaned = xml.check_tei(xml_doc, "fake_url")
    result = [
        (elem.tag, elem.text, elem.tail) for elem in cleaned.find(".//div/div").iter()
    ]
    ...
    expected = [
        ...
        ("p", "text1", None),
        ("quote", "text2", None),
        ("p", "has to be there", None),
    ]
    assert result == expected
    xml_doc = etree.fromstring(
        "<TEI><text><body><div><div>text1<p>text2</p>has to be there</div></div></body></text></TEI>"
    )
    cleaned = xml.check_tei(xml_doc, "fake_url")
    result = [
        (elem.tag, elem.text, elem.tail) for elem in cleaned.find(".//div/div").iter()
    ...
    htmlstring = html.fromstring(
        "<html><head/><body><div><h2><p>text</p></h2></div></body></html>"
    )
    ...
    htmlstring = html.fromstring(
        "<html><body><article><h1>title</h1><h2>subtitle</h2><p>text</p></article></body></html>"
    )
    extracted = extract(htmlstring, url="mocked", fast=True, output_format="xmltei")
    ...
    extracted = extract(htmlstring, url="mocked", fast=True, output_format="xmltei")
    assert (
        '<ab rend="h2" type="header">content<list rend="ul"><item>text1'
...
def test_htmlprocessing():
    ...
    options.tables = True
    assert (
        trafilatura.htmlprocessing.tree_cleaning(etree.Element("html"), options)
        is not None
    )
    ...
    assert '<head rend="h1">Test headline</head>' in extract(
        copy(mydoc), output_format="xml", config=ZERO_CONFIG, fast=True
    )
    ...
    mydoc = html.fromstring(
        "<html><body><p><span>A</span><span>B</span><span>C</span></p></body></html>"
    )
    for element in mydoc.iter("span"):
        xml.merge_with_parent(element)
    assert b"<p>A B C</p>" in etree.tostring(mydoc)
    mydoc = html.fromstring(
        "<html><body><p><span>A</span><span>B</span> tail<span>C</span></p></body></html>"
    )
    for element in mydoc.iter("span"):
        xml.merge_with_parent(element)
    ...
    # paywalls
    my_html = (
        '<html><body><main><p>1</p><p id="premium">2</p><p>3</p></main></body></html>'
    )
    ...
    node = etree.fromstring(
        "<p><ref target='url'><hi rend='#b'>bold</hi>inner</ref>outer</p>"
    )[0]
    ...
    node = etree.fromstring(
        "<p><ref target='url'>text<hi rend='#b'>bold</hi></ref>tail</p>"
    )[0]
    processed = trafilatura.htmlprocessing.handle_textnode(node, options)
    assert processed.tail == "tail" and processed.text == "text"
    ...
    node = html.fragment_fromstring(
        "<div><p><span>span</span> span tail</p> p tail </div>"
    )
...
def test_extraction_options():
    ...
    assert (
        extract(
            my_html, only_with_metadata=False, output_format="xml", config=ZERO_CONFIG
        )
        is not None
    )
    assert (
        extract(
            my_html, only_with_metadata=True, output_format="xml", config=ZERO_CONFIG
        )
        is None
    )
    ...
    # justext hardening
    assert (
        etree.tostring(try_justext(html.fromstring(my_html), None, "de")) == b"<body/>"
    )
    ...
    my_html = (
        "<html><body><p>" + "Text. " * 10 + "<embed>Test</embed></p></body></html>"
    )
    result = etree.tostring(try_readability(html.fromstring(my_html)))
    ...
    my_html = (
        "<html><head/><body>"
        + "<p>ABC def ghi jkl.</p>" * 1000
        + "<p>Posted on 1st Dec 2019<.</p></body></html>"
    )
    assert (
        bare_extraction(my_html, config=ZERO_CONFIG, with_metadata=True).date
        is not None
    )
...
def test_precision_recall():
    ...
    assert (
        extract(copy(my_document), favor_precision=True, config=ZERO_CONFIG, fast=True)
        is not None
    )
    assert (
        extract(copy(my_document), favor_recall=True, config=ZERO_CONFIG, fast=True)
        is not None
    )
    ...
    assert "teaser text" in extract(
        copy(my_document), favor_recall=True, config=ZERO_CONFIG, fast=True
    )
    assert "teaser text" not in extract(
        copy(my_document), config=ZERO_CONFIG, fast=True
    )
    assert "teaser text" not in extract(
        copy(my_document), favor_precision=True, config=ZERO_CONFIG, fast=True
    )
    ...
    result = extract(
        copy(my_document), favor_precision=False, config=ZERO_CONFIG, fast=True
    )
    ...
    my_document = html.fromstring(
        "<html><body><article><aside><p>Here is the text.</p></aside></article></body></html>"
    )
    result = extract(
        copy(my_document), favor_recall=False, config=ZERO_CONFIG, fast=True
    )
    ...
    my_document = html.fromstring(
        "<html><body><div><h2>Title</h2><small>Text.</small></div></body></html>"
    )
    result = extract(
        copy(my_document), favor_recall=True, config=ZERO_CONFIG, fast=False
    )
    assert len(result) > 0
...
def test_table_processing():
    ...
    assert (
        """<table><row><cell>text<head>more_text</head></cell></row></table>"""
        in result
    )
...
<td>they buy</td>
    ...
    assert extract(htmlstring, fast=True, output_format="txt").startswith(
    ...
    broken_table = html.fromstring(
        "<table><tr><p>text</p></tr><tr><td>cell</td></tr></table>"
    )
    processed_table = handle_table(broken_table, TAG_CATALOG, options)
    result = [el.tag for el in processed_table.iter()]
    assert result == [
 ...
 <dl>
  ...
  <dd>White cold drink</dd>
    ...
    result = [
        ...
        for child in processed_list.iter()
    ]
    assert result == ["list", "item", ("p", "text")]
    list_item_with_text_and_child = html.fromstring(
        "<list><item>text1<p>text2</p></item></list>"
    )
    ...
    list_with_text_outside_item = html.fromstring(
        "<list>header<item>text</item></list>"
    )
    ...
    list_item_with_child_and_tail = html.fromstring(
        "<list><item><p>text</p></item>tail</list>"
    )
    ...
    list_item_with_child_and_tail = html.fromstring(
        "<list><item><p>text</p>tail1</item>tail</list>"
    )
    processed_list = handle_lists(list_item_with_child_and_tail, options)
    item_element = processed_list[0]
    assert item_element.tail is not True
    ...
    assert item_element.tail is not True
    assert item_element[0].tail == "tail"
    list_item_with_tail_and_nested_list = html.fromstring(
        "<list><item><list><item>text</item></list></item>tail</list>"
    )
...
<span class="hljs-keyword">highlighted</span> more <span class="hljs-keyword">code</span>
    ...
    assert (
        "<code>$ pip install PyGithub</code>" in testresult
        and "quote" not in testresult
    )
...
<div class="w3-code notranslate pythonHigh"><span class="pythoncolor" style="color:black"><span class="pythonnumbercolor" style="color:red">
    ...
    expected = """<code>
  class Person:<lb/>\xa0 def __init__(self, name, age):<lb/>\xa0\xa0\xa0\x20
  self.name = name<lb/>\xa0\xa0\xa0 self.age = age<lb/><lb/>p1 = Person("John",\x20
  36)<lb/>
...
def test_lang_detection():
    ...
    samples = [
        ...
        {
            "html": "<html><body><p>Texte en français</p></body></html>",
            "expected": "fr",
        },
    ]
