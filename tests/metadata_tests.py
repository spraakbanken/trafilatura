...
def test_titles():
    ...
    tests = [
        (
            '<html><body><h3 class="title">T</h3><h3 id="title"></h3></body></html>',
            None,
        ),
        (
...
<script>(RLQ=window.RLQ||[]).push(function(){mw.config.set({"wgBackendResponseTime":112,"wgHostname":"mw2373"});});</script></html>""")
    ...
    metadata = extract_metadata(
        "<html><head><title> - Home</title></head><body/></html>"
    )
    assert metadata.title == "- Home"
    metadata = extract_metadata(
        "<html><head><title>My Title » My Website</title></head><body/></html>"
    )
...
def test_sitename():
    ...
    tests = [
        (
            '<html><head><meta name="article:publisher" content="@"/></head><body/></html>',
            None,
        ),
        (
            '<html><head><meta name="article:publisher" content="The Newspaper"/></head><body/></html>',
            "The Newspaper",
        ),
        (
            '<html><head><meta property="article:publisher" content="The Newspaper"/></head><body/></html>',
            "The Newspaper",
        ),
        (
...
def test_meta():
    ...
    doc = html.fromstring(
        "<html><p class='test'>a</p><p class='other'>b</p><p type='this'>cde</p></html>"
    )
    ...
    metadata = extract_metadata(
        "<html><head><title>" + "AAA" * 10000 + "</title></head></html>"
    )
    assert metadata.title.endswith("…") and len(metadata.title) == 10000
    assert (
        extract_metadata(
            '<html><head><meta otherkey="example" content="Unknown text"/></head></html>'
        )
        is not None
    )
    assert (
        extract_metadata(
            "<html><head><title></title><title></title><title></title></head></html>"
        )
        is not None
    )
