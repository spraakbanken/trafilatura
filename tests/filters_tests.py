...
def test_filters():
    ...
    if LANGID_FLAG is True:
        # main text
        assert (
            language_filter("Hier ist ein Text auf Deutsch", "", "de", SAMPLE_META)[0]
            is False
        )
        assert (
            language_filter("Hier ist ein Text auf Deutsch", "", "en", SAMPLE_META)[0]
            is True
        )
        # comments
        assert (
            language_filter(
                "Hier ist ein Text.",
                "Die Kommentare sind aber etwas länger.",
                "de",
                SAMPLE_META,
            )[0]
            is False
    ...
    assert (
        check_html_lang(
            html.fromstring("<html><body></body></html>"), target_language="en"
        )
        is True
    )
    # text + lang
    ...
    if LANGID_FLAG is True:
        assert (
            extract(
                html.fromstring(
                    '<html lang="en-US"><body>' + my_p * 50 + "</body></html>"
                ),
                no_fallback=True,
                target_language="en",
            )
        ...
        assert (
            extract(
                html.fromstring(
                    '<html lang="de-DE"><body>' + my_p * 50 + "</body></html>"
                ),
                no_fallback=False,
                target_language="de",
            )
...
def test_prune_xpath():
    ...
    def doc3():
        ...
        return html.fromstring(
            "<html><body>" + my_h1 + my_h2 + my_p * 50 + "</body></html>"
        )
