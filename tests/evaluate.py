...
def run_justext(htmlbinary):
    ...
    paragraphs = justext.justext(
        htmlbinary, JT_STOPLIST, 50, 200, 0.1, 0.2, 0.2, 200, True
    )  # stop_words
...
def run_magic_html(htmlbinary):
    ...
    return run_bs4(
        magic_html_extractor.extract(convert_to_str(htmlbinary), base_url="").get(
            "html"
        )
    )
...
class Evaluation:
    ...
    def evaluate_result(self, result, item):
        ...
        if self.evaltype == "chunks":
            ...
            if result is not None and isinstance(result, str):
                true_positives = sum(
                    1 for to_include in item["with"] if to_include in result
                )
                false_negatives = len(item["with"]) - true_positives
                false_positives = sum(
                    1 for to_exclude in item["without"] if to_exclude in result
                )
    ...
    def create_df(self):
        ...
        columns = (
            ["algorithm", "version"]
            + self.metrics
            + ["time difference", "skipped instances"]
        )
        rows = []
        for algo in self.algorithms:
            algo_version = (
                version(ALGORITHMS[algo]["library"])
                if ALGORITHMS[algo]["library"] != "-"
                else "-"
            )
    ...
    def print_scores(self):
        ...
        for algo in self.results:
            ...
            result = separator.join(
                [f"{m}: {self.output_df.loc[algo][m]:.3f}" for m in self.metrics]
            )
            print(
                result,
                f"time: {self.output_df.loc[algo]['time difference']:.2f}",
                sep=separator,
            )
            print()
...
def cmdparser():
    ...
    parser.add_argument(
        "--small", action="store_true", help="Evaluate trafilatura and baselines only."
    )
    parser.add_argument(
        "--all", action="store_true", help="Evaluate all available algorithms."
    )
    # file path, metrics and algorithms as default
    parser.add_argument(
        "--testfile", default="evaldata.json", help="File path to the test data."
    )
    parser.add_argument(
        "--metrics",
        nargs="+",
