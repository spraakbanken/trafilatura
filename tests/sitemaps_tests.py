...
def test_extraction():
    ...
    sitemap = sitemaps.SitemapObject(
        "https://example.org", "example.org", ["https://example.org/sitemap.xml"]
    )
    ...
    assert len(sitemap.sitemap_urls) == 1 and sitemap.urls == [
        "https://mydomain.wordpress.com/1"
    ]

    sitemap = sitemaps.SitemapObject(
        "https://programtalk.com",
        "programtalk.com",
        ["https://programtalk.com/sitemap.xml"],
    )
    sitemap.handle_link(
    ...
    # safety belts
    assert (
        sitemaps.is_plausible_sitemap("http://example.org/sitemap.xml.gz", None)
        is False
    )
    assert (
        sitemaps.is_plausible_sitemap(
            "http://example.org/sitemap.xml.gz", b"\x1f\x8bABC"
        )
        is False
    )
    assert (
        sitemaps.is_plausible_sitemap("http://example.org/sitemap.xml", "ABC") is False
    )
    assert (
        sitemaps.is_plausible_sitemap(
            "http://test.org/sitemap.xml", "<!DOCTYPE html><html><body/></html>"
        )
        is False
    )
    assert (
        sitemaps.is_plausible_sitemap(
            "http://test.org/sitemap", "<!DOCTYPE html><html><body/></html>"
        )
        is False
    )
    # invalid
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        teststring = f.read()
    assert (
        sitemaps.is_plausible_sitemap("http://sitemaps.org/sitemap.xml", teststring)
        is True
    )
    sitemap = sitemaps.SitemapObject(baseurl, domain, [])
    ...
    sitemap.process()
    assert (sitemap.sitemap_urls, sitemap.urls) == (
        [],
        ["http://www.test.org/english/page.html"],
    )
    ...
    assert (
        sitemaps.is_plausible_sitemap("http://example.org/sitemap.xml.gz", teststring)
        is True
    )
    sitemap = sitemaps.SitemapObject(baseurl, domain, [url])
    sitemap.content = teststring
    ...
    assert (
        sitemaps.is_plausible_sitemap(
            "http://example.org/sitemap.xml.gz?value=1", teststring
        )
        is True
    )
    ...
    assert (sitemap.sitemap_urls, sitemap.urls) == (
        [],
        ["https://test.org/1", "https://test.org/2"],
    )
    ...
    assert (sitemap.sitemap_urls, sitemap.urls) == (
        [],
        ["https://test.org/en/1", "https://test.org/en/2"],
    )
...
def test_robotstxt():
    ...
    assert sitemaps.extract_robots_sitemaps(
        "sitemap: https://example.org/sitemap.xml", baseurl
    ) == ["https://example.org/sitemap.xml"]


def test_whole():
    "Test whole process."
    results = sitemaps.sitemap_search(
        "https://www.sitemaps.org", target_lang="de", max_sitemaps=1
    )
